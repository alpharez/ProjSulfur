...
-(id)init {
    self = [super init];
    _zone1 = TCOD_map_new(SCREEN_WIDTH, SCREEN_HEIGHT);
    ...
    [self createRoomAtX1:35 andY1:20 andX2:60 andY2:35];
...
-(void)vline_down:(int) x y:(int) y {
    while (y < SCREEN_HEIGHT && !TCOD_map_is_walkable(_zone1, x, y)) {
        TCOD_map_set_properties(_zone1,x,y,true,true); /* ground */
        y++;
    }
}
...
-(void)hline_right:(int) x y:(int) y {
    while (x < SCREEN_WIDTH && !TCOD_map_is_walkable(_zone1, x, y)) {
        TCOD_map_set_properties(_zone1,x,y,true,true); /* ground */
        x++;
    }
}
...
-(void)createRooms:(int) num {
    ...
    int roomWidth = 8;
    int roomHeight = 10;
    int roomBuffer = 25; // ### HAVE A BUG HERE
    for( int i=0; i < num; i++ ) {
        int randX = TCOD_random_get_int(NULL, 1, SCREEN_WIDTH - roomWidth);
        int randY = TCOD_random_get_int(NULL, 1, SCREEN_HEIGHT - roomHeight - roomBuffer);
...
-(void)render:(int)px y:(int)py {
    ...
    for (int mapX = 0; mapX < SCREEN_WIDTH; mapX++) {
        for (int mapY = 0; mapY < SCREEN_HEIGHT; mapY++) {
            ...
            if(!visible) {
                if( tiles[mapX][mapY]) {
                    TCOD_console_set_char_background(NULL, mapX, mapY,
                ...
                TCOD_console_set_char_background(NULL, mapX, mapY, base, TCOD_BKGND_SET);
            }
        }
    }
}
